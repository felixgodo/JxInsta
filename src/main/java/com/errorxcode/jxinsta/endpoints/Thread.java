...
public class Thread {
    ...
    private void fetchThread(int count) throws InstagramException, IOException {
        ...
        try (var res = Utils.call(req)) {
            ...
            if (!status.equals("ok")) {
                throw new InstagramException("Error in getting thread: " + json, InstagramException.Reason.UNKNOWN);
            }
    ...
    public String sendMessage(String message, boolean forwarded) throws IOException, InstagramException {
        ...
        try (var res = Utils.call(req)) {
            ...
            var status = json.getString("status");
            if (!status.equals("ok")) {
                throw new InstagramException("Error in sending message: " + json, InstagramException.Reason.UNKNOWN);
            }
    ...
    private long uploadPhoto(@NotNull File photo) throws InstagramException, IOException {
        if (!photo.exists()) {
            throw new InstagramException("File does not exist", InstagramException.Reason.UNKNOWN);
        } else {
            var extension = photo.getName().substring(photo.getName().lastIndexOf("."));
            if (!extension.equals(".jpg") && !extension.equals(".png") && !extension.equals(".jpeg")) {
                throw new InstagramException("Invalid file type", InstagramException.Reason.UNKNOWN);
            }
        }
    ...
    public void sendPhoto(@NotNull File photo) throws IOException, InstagramException {
        ...
        try (var res = Utils.call(req)) {
            ...
            var status = json.getString("status");
            if (!status.equals("ok")) {
                throw new InstagramException("Error in sending photo: " + json, InstagramException.Reason.UNKNOWN);
            }
        }
    }

    public void markSeen() throws InstagramException, IOException {
        var req = Utils.createPostRequest(authInfo,
                "direct_v2/threads/" + threadId + "/items/" + messages[0].id + "/seen/",
                null);
        try (var res = Utils.call(req)) {
            var json = new JSONObject(res.body().string());
            var status = json.getString("status");
            if (!status.equals("ok")) {
                throw new InstagramException("Error in marking seen: " + json, InstagramException.Reason.UNKNOWN);
            }
        }
    }
    ...
    public void unsend(@NotNull String messageId) throws InstagramException, IOException {
        var req = Utils.createPostRequest(authInfo,
                "direct_v2/threads/" + threadId + "/items/" + messageId + "/delete/",
                null);
        try (var res = Utils.call(req)) {
            var json = new JSONObject(res.body().string());
            var status = json.getString("status");
            if (!status.equals("ok")) {
                throw new InstagramException("Error in unsent: " + json, InstagramException.Reason.UNKNOWN);
            }
        }
    }
    ...
    protected static void delete(@NotNull AuthInfo authInfo,
            @NotNull String threadId) throws InstagramException, IOException {
        var req = Utils.createPostRequest(authInfo,
                "direct_v2/threads/" + threadId + "/hide/",
                Map.of("should_move_future_requests_to_spam", false));
        try (var res = Utils.call(req)) {
            var json = new JSONObject(res.body().string());
            var status = json.getString("status");
            if (!status.equals("ok")) {
                throw new InstagramException("Error in deleting thread: " + json, InstagramException.Reason.UNKNOWN);
            }
        }
    }
