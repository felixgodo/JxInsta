...
public class Post {
    ...
    public static Post getPost(@NotNull String url) throws InstagramException, IOException {
        ...
        try (var res = Utils.call(req)) {
            ...
            post.caption = shortcode_media.getJSONObject("edge_media_to_caption")
                    .getJSONArray("edges")
                    .getJSONObject(0)
                    .getJSONObject("node")
                    .getString("text");
            ...
            post.download_url = post.isVideo ? shortcode_media.getString("video_url") : shortcode_media.getString(
                    "display_url");
            return post;
    ...
    public List<Comment> getComments(int count) throws IOException, InstagramException {
        var req = Utils.createGetRequest("media/" + id + "/comments?can_support_threading=true&permalink_enabled=false",
                authInfo);
        try (var res = Utils.call(req)) {
            var json = new JSONObject(res.body().string());
            if (json.has("status") && !json.getString("status").equals("ok")) {
    ...
    public static class Comment {
        ...
        public void reply(@NotNull String reply) throws InstagramException, IOException {
            var req = Utils.createPostRequest(authInfo,
                    "web/comments/" + mediaId + "/add/",
                    Map.of("comment_text", reply, "replied_to_comment_id", id));
            try (var res = Utils.call(req)) {
                var json = new JSONObject(res.body().string());
                if (json.has("status") && !json.getString("status").equals("ok")) {
                    throw new InstagramException(json.getString("message"), InstagramException.Reason.UNKNOWN);
